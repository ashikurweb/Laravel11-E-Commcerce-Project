    ...
    <section class="product-single container">
      <div class="row">
            ...
            <div class="product-single__image">
              <div class="swiper-container">
                <div class="swiper-wrapper">
                  ...
                  @foreach (explode(',', $product->images) as $imgGallery)
                  <div class="swiper-slide product-single__image-item">
                    <img loading="lazy" class="h-auto" src="{{ asset('uploads/products/' . $imgGallery) }}" width="674"
                      height="674" alt="" />
                    <a data-fancybox="gallery" href="{{ asset('uploads/products/' . $imgGallery) }}" data-bs-toggle="tooltip"
                      data-bs-placement="left" title="Zoom">
                      <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <use href="#icon_zoom" />
                      </svg>
              ...
              <div class="swiper-container">
                <div class="swiper-wrapper">
                  <div class="swiper-slide product-single__image-item"><img loading="lazy" class="h-auto"
                      ...
                      @foreach (explode(',', $product->images) as $imgGallery)
                        <div class="swiper-slide product-single__image-item">
                            <img loading="lazy" class="h-auto"src="{{ asset('uploads/products/thumbnails/'. $imgGallery) }}" width="104" height="104" alt="" />
                        </div>
                      @endforeach
                </div>
              </div>
        ...
        <div class="col-lg-5">
          ...
          @else
          <form name="addtocart-form" action="{{ route('cart.addToCart') }}" method="POST">
            @csrf
          ...
          <div class="swiper-wrapper">
            ...
            <div class="swiper-slide product-card">
              <div class="pc__img-wrapper">
                <a href="{{ route('shop.product-details', $relatedproduct->slug) }}">
                  <img loading="lazy" src="{{ asset('uploads/products/' . $relatedproduct->image) }}" width="330" height="400"
                    alt="{{ $relatedproduct->name }}" class="pc__img">
                    @foreach (explode(',', $relatedproduct->images) as $imgGallery)
                    <img loading="lazy" src="{{ asset('uploads/products/' . $imgGallery) }}" width="330" height="400" alt="{{ $relatedproduct->name }}" class="pc__img pc__img-second">
                    @endforeach
                </a>
                @if (Cart::instance('cart')->content()->where('id', $product->id)->count() > 0)
                ...
                @else
                <form action="{{ route('cart.addToCart') }}" method="POST" name="addtocart-form">
                    @csrf
                    <input type="hidden" name="id" value="{{ $relatedproduct->id }}">
                    <input type="hidden" name="quantity" value="1">
                    <input type="hidden" name="name" value="{{ $relatedproduct->name }}">
                    <input type="hidden" name="price" value="{{ $relatedproduct->sale_price == '' ? $relatedproduct->regular_price : $relatedproduct->sale_price }}">
              ...
              <div class="pc__info position-relative">
                ...
                <h6 class="pc__title"><a href="{{ route('shop.product-details', $relatedproduct->slug) }}">{{ $relatedproduct->name }}</a></h6>
                <div class="product-card__price d-flex">
                  <span class="money price">
                    @if ($product->sale_price)
                        <s>${{ $product->regular_price }}</s> ${{ $relatedproduct->sale_price }}
                    @else
                        ${{ $product->regular_price }}
                    @endif
                  </span>
